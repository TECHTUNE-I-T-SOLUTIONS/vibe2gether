...
CREATE TABLE public.subscription_services (
  ...
  duration_unit character varying NOT NULL DEFAULT 'month'::character varying CHECK (duration_unit::text = ANY (ARRAY['day'::character varying, 'week'::character varying, 'month'::character varying, 'year'::character varying]::text[])),
  featured_services ARRAY NOT NULL DEFAULT '{}'::text[],
  location_name character varying,
  terms text,
  image_url text,
  ...
  CONSTRAINT subscription_services_created_by_fkey FOREIGN KEY (created_by) REFERENCES public.admins(id)
);
CREATE TABLE public.user_subscription_purchases (
  ...
  amount numeric NOT NULL,
  currency character varying NOT NULL DEFAULT 'NGN'::character varying,
  status character varying NOT NULL DEFAULT 'pending'::character varying CHECK (status::text = ANY (ARRAY['pending'::character varying, 'active'::character varying, 'expired'::character varying, 'cancelled'::character varying]::text[])),
  ...
  paystack_reference character varying UNIQUE,
  ...
  created_at timestamp with time zone NOT NULL DEFAULT now(),
  updated_at timestamp with time zone NOT NULL DEFAULT now(),
  CONSTRAINT user_subscription_purchases_pkey PRIMARY KEY (id),
  CONSTRAINT user_subscription_purchases_user_id_fkey FOREIGN KEY (user_id) REFERENCES public.users(id),
  CONSTRAINT user_subscription_purchases_service_id_fkey FOREIGN KEY (service_id) REFERENCES public.subscription_services(id)
);