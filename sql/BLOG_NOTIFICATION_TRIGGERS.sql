...
BEGIN
  ...
  -- Notify post author (if not the commenter themselves)
  IF v_post_author_id IS NOT NULL AND NEW.user_id != v_post_author_id THEN
    -- Check if this is a reply (parent_id is not null)
    IF NEW.parent_id IS NULL THEN
      ...
      VALUES (
        ...
        v_commenter_name || ' commented on "' || v_post_title || '"',
        NEW.user_id,
        NEW.post_id,
        'blog_post',
        now()
...
BEGIN
  ...
  IF NEW.parent_id IS NOT NULL THEN
    ...
    IF v_parent_comment_author_id IS NOT NULL AND NEW.user_id != v_parent_comment_author_id THEN
      INSERT INTO public.notifications (user_id, type, title, message, actor_id, reference_id, reference_type, created_at)
      VALUES (
...
BEGIN
  ...
  -- Notify post author (if not the liker themselves)
  IF v_post_author_id IS NOT NULL AND NEW.user_id != v_post_author_id THEN
    INSERT INTO public.notifications (user_id, type, title, message, actor_id, reference_id, reference_type, created_at)
    VALUES (
      v_post_author_id,
      ...
      v_liker_name || ' liked "' || v_post_title || '"',
      NEW.user_id,
      NEW.post_id,
      'blog_post',
      now()
...
-- ============================================================================
-- TRIGGER 4: Notify comment author when someone likes their blog comment
-- ============================================================================
...
BEGIN
  -- Get comment author ID and content
  SELECT user_id, content INTO v_comment_author_id, v_comment_content
  FROM public.blog_comments
  ...
  -- Notify comment author (if not the liker themselves)
  IF v_comment_author_id IS NOT NULL AND NEW.user_id != v_comment_author_id THEN
    INSERT INTO public.notifications (user_id, type, title, message, actor_id, reference_id, reference_type, created_at)
    VALUES (
      v_comment_author_id,
      'blog_comment_like',
...
-- ============================================================================
-- This script creates:
-- 1. blog_likes table - tracks who liked which blog posts
-- 2. blog_comment_likes table - tracks who liked which comments
-- 3. 4 Triggers:
--    - notify_on_blog_comment: Notifies post author & admin on new comments
--    - notify_on_comment_reply: Notifies parent comment author on replies
--    - notify_on_blog_like: Notifies post author when post is liked
--    - notify_on_blog_comment_like: Notifies comment author when comment is liked
